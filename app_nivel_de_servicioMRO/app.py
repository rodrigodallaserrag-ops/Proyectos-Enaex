...
with tab_trazabilidad:
    ...
    st.markdown(
        ...
        "las solicitudes de compra iniciales, sus agregadas y su salida a SAP ERP (Solped 600)[cite: 10]."
    )
    ...
    with traz_col2:
        empresa_id = st.text_input("Empresa compradora (ID)", value=trazabilidad.EMPRESA_POR_DEFECTO)
    ...
    if "df_trazabilidad_limpio" in st.session_state and "df_trazabilidad_cadena" in st.session_state:
        ...
        with tab_res:
            ...
            st.caption("Esta información servirá de cruce directo con la base ME5A[cite: 10].")
