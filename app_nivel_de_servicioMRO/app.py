...
st.set_page_config(
    page_title="Dx Compradores - Nivel de Servicio", layout="wide"
)
...
def determinar_tipo_ariba(row):
  ...
  - ⚙️ SAP ERP: Serie 1 (100...), Serie 19, CL... - ⚪ SAP MRP: Serie 5 (500...) o
  marca de Solped MRP. - 🟡 ARIBA DIRECTA: Flujo directo / automatizado. - 🟢
  ARIBA CATALOGADA: Serie 6 con código de material/catálogo. - 🔵 ARIBA NO
  CATALOGADA: Serie 6 sin código de material o en Trazabilidad.
  """
  ...
  tiene_material = bool(
      material and material.lower() not in ["nan", "none", "n/a", "-", "0"]
  )
  es_mrp_flag = (
      str(row.get("Solped MRP", "")).strip().lower()
      in ["sí", "si", "true", "mrp", "1"]
  )
  en_trazabilidad = bool(
      row.get("En_Trazabilidad", False) or row.get("En Trazabilidad", False)
  )
...
def aplicar_calculo_nivel_servicio(
    df, metrica_no_cat="SLA Comprador (Desde Liberación PR Inicial)"
):
  """Recalcula la columna 'Nivel de Servicio' ajustando Ariba No Catalogada

  según la Fase 0 de Trazabilidad.
  """
  df = df.copy()

  # Fechas base globales
  f_solicitud = pd.to_datetime(df["Fecha de solicitud"], errors="coerce")
  f_pedido = pd.to_datetime(df["Fecha de pedido"], errors="coerce")

  # SLA Estándar (Días entre solicitud y pedido)
  df["SLA_Estandar"] = (f_pedido - f_solicitud).dt.days

  # Columnas trazables si existen en el merge
  f_lib_inicial = (
      pd.to_datetime(df["Fecha liberación PR inicial"], errors="coerce")
      if "Fecha liberación PR inicial" in df.columns
      else f_solicitud
  )
  f_crea_inicial = (
      pd.to_datetime(df["Fecha creación PR inicial"], errors="coerce")
      if "Fecha creación PR inicial" in df.columns
      else f_solicitud
  )
  f_po = (
      pd.to_datetime(df["Fecha PO"], errors="coerce")
      if "Fecha PO" in df.columns
      else f_pedido
  )

  df["SLA_Comprador_NoCat"] = (
      f_po.fillna(f_pedido) - f_lib_inicial.fillna(f_solicitud)
  ).dt.days
  df["Lead_Time_E2E_NoCat"] = (
      f_po.fillna(f_pedido) - f_crea_inicial.fillna(f_solicitud)
  ).dt.days

  es_no_cat = df["Tipo Ariba"] == "🔵 ARIBA NO CATALOGADA"

  if "SLA Comprador" in metrica_no_cat:
    df["Nivel de Servicio"] = df["SLA_Estandar"]
    df.loc[es_no_cat, "Nivel de Servicio"] = df.loc[
        es_no_cat, "SLA_Comprador_NoCat"
    ].fillna(df.loc[es_no_cat, "SLA_Estandar"])
  else:
    df["Nivel de Servicio"] = df["SLA_Estandar"]
    df.loc[es_no_cat, "Nivel de Servicio"] = df.loc[
        es_no_cat, "Lead_Time_E2E_NoCat"
    ].fillna(df.loc[es_no_cat, "SLA_Estandar"])
...
with st.sidebar:
  ...
  if modo == "OneDrive (automático)":
    ...
    archivo_data = st.file_uploader(
        ...
        type=["xlsx", "parquet"],
        help=(
            "Si ya lo convertiste en la pestaña 'Preparar datos', sube el"
            " .parquet."
        ),
    )
    archivo_resp_grupo = st.file_uploader(
        "Responsable_Grupo_Compras.xlsx", type="xlsx"
    )
...
with st.sidebar:
  ...
  fecha_corte = st.date_input(
      "Fecha de corte del reporte (FechaCorteReporte)",
      value=pd.Timestamp.today(),
  )
  metrica_no_cat = st.radio(
      "Medición Ariba No Catalogada",
      [
          "SLA Comprador (Desde Liberación PR Inicial)",
          "Lead Time E2E (Desde Creación Solped Inicial)",
      ],
      index=0,
      help=(
          "SLA Comprador descuenta la Fase 0 de aprobaciones en planta. Lead"
          " Time E2E mide el tiempo total percibido por el usuario."
      ),
  )
  st.caption(
      f"SLA Target: {config.SLA_DIAS_ERP_MRP} días ERP/MRP ·"
      f" {config.SLA_DIAS_ARIBA} días Ariba"
  )
...
if st.session_state.get("_clave_pipeline") != clave_actual:
  ...
  # Asignación de categoría y recalculo de Nivel de Servicio con la Fase 0
  df_calculado["Tipo Ariba"] = df_calculado.apply(determinar_tipo_ariba, axis=1)
  df_calculado = aplicar_calculo_nivel_servicio(
      df_calculado, metrica_no_cat=metrica_no_cat
  )
...
NS_MIN_GLOBAL = (
    int(df["Nivel de Servicio"].min())
    if len(df) and pd.notna(df["Nivel de Servicio"].min())
    else 0
)
NS_MAX_GLOBAL = (
    int(df["Nivel de Servicio"].max())
    if len(df) and pd.notna(df["Nivel de Servicio"].max())
    else 100
)
...
def _snapshot(nombre, d):
  ...
  metricas_por_etapa.append((
      ...
      f"{n:,}",
      f"{pct:.0f}%",
      f"{prom:.0f}" if pd.notna(prom) else "-",
      f"{pos_oc:,}",
  ))
...
with c3:
  tipos_ariba = st.multiselect(
      "Origen / Tipo Solicitud", sorted(df["Tipo Ariba"].dropna().unique())
  )
...
# ---- Estado Solped ----
st.caption(
    "Estado Solped (el filtro de fecha de abajo solo aplica dentro de 'Pedido"
    " completo')"
)
h1, h2, h3, h4 = st.columns(4)
with h1:
  estados = st.multiselect(
      "Estado Solped", sorted(df_f["Estado Solped"].dropna().unique())
  )
...
checkpoints.append((
    "3a. Sin pedido (antes de jerarquía)",
    (df_f["Estado Solped"] == "Sin pedido").sum(),
))
checkpoints.append((
    "3b. Pedido incompleto (antes de jerarquía)",
    (df_f["Estado Solped"] == "Pedido incompleto").sum(),
))
checkpoints.append((
    "3c. Pedido completo (antes de jerarquía)",
    (df_f["Estado Solped"] == "Pedido completo").sum(),
))
...
with h2:
  años = st.multiselect(
      "Año", sorted(df_pedido_completo["Año"].dropna().unique().astype(int))
  )
with h3:
  _base_mes = (
      df_pedido_completo[df_pedido_completo["Año"].isin(años)]
      if años
      else df_pedido_completo
  )
  meses = st.multiselect(
      "Mes", sorted(_base_mes["Mes"].dropna().unique().astype(int))
  )
with h4:
  _base_dia = _base_mes[_base_mes["Mes"].isin(meses)] if meses else _base_mes
  fechas_disponibles = sorted(
      _base_dia["Fecha de pedido"].dt.date.dropna().unique()
  )
...
checkpoints.append((
    "4a. Sin pedido tras jerarquía",
    (df_f["Estado Solped"] == "Sin pedido").sum(),
))
checkpoints.append((
    "4b. Pedido incompleto tras jerarquía",
    (df_f["Estado Solped"] == "Pedido incompleto").sum(),
))
checkpoints.append((
    "4c. Pedido completo tras jerarquía",
    (df_f["Estado Solped"] == "Pedido completo").sum(),
))
...
c4, c5 = st.columns(2)
with c4:
  solped_mrp = st.multiselect(
      "Solped MRP", sorted(df_f["Solped MRP"].dropna().unique())
  )
with c5:
  cumple = st.multiselect(
      "Nivel de Servicio (Cumple)", sorted(df_f["Cumple"].dropna().unique())
  )
...
if len(df_f):
  ...
  with f2:
    if excluir_negativos:
      ...
      st.caption(
          f"Rango aplicado: 0 a {NS_MAX_GLOBAL:,} días (negativos excluidos)"
      )
...
checkpoints.append(
    ("7. Tras filtro Nivel de Servicio (RESULTADO FINAL)", len(df_f))
)
...
with st.expander("🔍 Diagnóstico de filtrado (para comparar contra el pbix)"):
  ...
  st.download_button(
      "Descargar detalle filtrado (CSV)", csv, "detalle_filtrado.csv", "text/csv"
  )
...
pedidos_distintos = df_f["Pedido"].nunique() + (
    1 if df_f["Pedido"].isna().any() else 0
)
...
def tarjeta(
    titulo: str,
    valor: str,
    fondo: str = "rgba(64,75,85,0.07)",
    borde: str = "rgba(64,75,85,0.35)",
) -> str:
  return (
      f'<div style="background:{fondo};border:1.5px solid'
      f' {borde};border-radius:8px;padding:14px 18px;text-align:center;"><div'
      ' style="font-size:0.78rem;color:#404B55;font-weight:600;letter-spacing:.03em;text-transform:uppercase;opacity:.85;margin-bottom:4px;">'
      f"{titulo}</div><div"
      ' style="font-size:2rem;font-weight:700;color:#404B55;line-height:1.1;">'
      f"{valor}</div></div>"
  )
...
f_pct, b_pct = (
    (VERDE, VERDE_BORDE) if pct_cumplimiento >= 85 else (ROJO, ROJO_BORDE)
)

t1, t2, t3 = st.columns(3)
with t1:
  st.markdown(
      tarjeta("Promedio días de gestión", txt_dias, f_dias, b_dias),
      unsafe_allow_html=True,
  )
with t2:
  st.markdown(
      tarjeta("% Cumplimiento", f"{pct_cumplimiento:.0f}%", f_pct, b_pct),
      unsafe_allow_html=True,
  )
with t3:
  st.markdown(
      tarjeta("OC generadas", f"{pedidos_distintos:,}"), unsafe_allow_html=True
  )
...
def tabla_enaex(
  ...
  for i, r in enumerate(tabla.itertuples(index=False)):
    ...
    if es_total:
      estilo_fila = (
          f"background:{ENAEX_GRIS};color:#fff;font-weight:700;border-top:2px"
          f" solid {ENAEX_ROJO};"
      )
    else:
    ...
    for j, c in enumerate(cols):
      ...
      celdas.append(
          f'<td style="padding:{pad};text-align:{align};border-bottom:1px solid'
          f' #e3e5e8;white-space:nowrap;">{_fmt(c, r[j])}</td>'
      )
  ...
  encabezados = "".join(
      f'<th style="padding:{pad_th};text-align:{"left" if j == 0 else "right"};background:{ENAEX_GRIS};color:#fff;font-weight:600;font-size:{fuente_th};letter-spacing:.02em;position:sticky;top:0;z-index:2;white-space:nowrap;">{abrev.get(c, c) if compacta else c}</th>'
      for j, c in enumerate(cols)
  )

  tabla_html = (
      f'<table style="width:100%;min-width:{"340px" if compacta else "auto"};border-collapse:collapse;font-size:{fuente};font-family:inherit;border:1px'
      f" solid #d8dbdf;"><thead><tr>{encabezados}</tr></thead><tbody>{''.join(filas)}</tbody></table>"
  )

  alto = f"max-height:{max_height}px;overflow-y:auto;" if max_height else ""
  return (
      f'<div style="{alto}overflow-x:auto;border:1px solid'
      f' #d8dbdf;border-radius:4px;">{tabla_html}</div>'
  )
...
st.caption(
    ...
    " compradores responsables de cada grupo, en vez de concentrarse en el"
    " responsable de MRP."
)
col_comprador = (
...
tabla_comprador = transform.agregar_fila_total(
    tabla_comprador, df_f, [col_comprador]
)
...
with vc2:
  ...
  tabla_detalle = tabla_detalle.sort_values(
      "Pos. OC generadas", ascending=False
  )
  tabla_detalle = transform.agregar_fila_total(
      tabla_detalle, df_f, cols_detalle
  )
  st.markdown(
      tabla_enaex(tabla_detalle, max_height=300, compacta=True),
      unsafe_allow_html=True,
  )
...
with st.expander("Ver detalle de solicitudes", expanded=False):
  st.caption(
      "La columna **Comentario** es editable: escribe ahí y se incluirá en el"
      " Excel de registro."
  )
  detalle_editado = st.data_editor(
      ...
      column_config={
          "Tipo Ariba": st.column_config.TextColumn(
              "Origen / Tipo Solicitud", width="medium"
          ),
          "Comentario": st.column_config.TextColumn(
              "Comentario",
              help="Anotación libre para el registro semanal",
              width="medium",
          ),
      },
...
def generar_excel(detalle_df: pd.DataFrame) -> bytes:
  ...
  def ajustar_ancho(ws, tabla, col_inicio=1, extra=3):
    for j, col in enumerate(tabla.columns):
      ...
      ws.column_dimensions[get_column_letter(col_inicio + j)].width = min(
          max(largos) + extra, 45
      )
  ...
  ws.title = "Resumen"
  ws["A1"] = (
      "Nivel de Servicio MRO — Registro semana"
      f" {num_semana:02d}/{semana_ref.year}"
  )
  ...
  ws.cell(row=5, column=1).font = Font(
      name=fuente_base, bold=True, size=12, color=rojo
  )
  ajustar_ancho(ws, resumen)

  fila = escribir_hoja(
      ws, "Por comprador", tabla_comprador, fila_inicio=fila + 2
  )
  fila = escribir_hoja(
      ws, "Por centro logístico", tabla_fija, fila_inicio=fila + 2
  )
...
st.caption(
    "Prepara el reporte completo (indicadores, tablas y detalle con"
    f" comentarios) como **{nombre_archivo}**."
)

clave_excel = (
    len(df_f),
    int(pd.util.hash_pandas_object(detalle_editado["Comentario"].fillna("")).sum()),
    pd.Timestamp(fecha_corte),
)
...
with col_desc:
  ...
  if excel_listo and excel_desactualizado:
    st.caption(
        "⚠️ Los filtros cambiaron desde que preparaste este Excel — vuelve a"
        " preparar para reflejar la selección actual."
    )
  if excel_listo:
    st.download_button(
    ...
    st.caption(
        'Haz clic en "Preparar Excel" para generar el archivo de descarga.'
    )
